...
function _constraints_satisfaction(p::Int32,num::Vector{Float32})
    ...
    num[1] == 0f0 && (p != zero32 || num[2] != 0f0 || num[3] != 0f0) && throw(ArgumentError("The first entry of the input array can be 0 only if all the other entries and the degree are nil too."))
    return true
end
