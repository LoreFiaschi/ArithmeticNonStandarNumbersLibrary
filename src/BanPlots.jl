...
function plot(x::Vector{Ban}, y::Vector{Ban}; kwargs...)
    ...
    if isbits(y[1])
        launch_plot_for_isbits(x,y,kwargs)
    else
        launch_plot_for_non_isbits(x,y,kwargs)
    end
end     
#################################################################
# Standard Ban Plot
#################################################################
function launch_plot_for_non_isbits(x::Vector{Ban}, y::Vector{Ban}, kwargs)
        ...
        if rlplot #if reinforcement learning reward plot flag is active
            for j in intervals_y
                ...
                if degy[j[1]]==max_degree_y
                    for n=1:BAN.SIZE
                        if j[1]==j[2]
                            axs[n].scatter([z.num[1] for z in x[j[1]:j[2]]], [z.num[n] for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                        else   
                    ...
                    for n=1:max_degree_y-degy[j[1]]
                        if j[1]==j[2]
                            axs[n].scatter([z.num[1] for z in x[j[1]:j[2]]], 0, s=3,c = :steelblue);
                        else   
                    ...
                    for n=max_degree_y-degy[j[1]]+1:BAN.SIZE
                        ind+=1
                        if j[1]==j[2]
                            axs[n].scatter([z.num[1] for z in x[j[1]:j[2]]], [z.num[ind] for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                        else   
            ...
            for j in intervals_y
                if j[1]==j[2]
                    axs[max_degree_y-degy[j[1]]+1].scatter([z.num[1] for z in x[j[1]:j[2]]], [z.num[1] for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                else   
...
#################################################################
# Size 3 isbits Ban Plot
#################################################################
function launch_plot_for_isbits(x::Vector{Ban}, y::Vector{Ban}, kwargs)
    degx = [degree(x[i]) for i in eachindex(x)];
    degy = [degree(y[i]) for i in eachindex(y)];
    ...
    for i=1:num_subplot_x
        ...
        if rlplot #if reinforcement learning reward plot flag is active
            for j in intervals_y
                ...
                if degy[j[1]]==max_degree_y
                        if j[1]==j[2]
                            axs[1].scatter([z.num1 for z in x[j[1]:j[2]]], [z.num1 for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                            axs[2].scatter([z.num1 for z in x[j[1]:j[2]]], [z.num2 for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                            axs[3].scatter([z.num1 for z in x[j[1]:j[2]]], [z.num3 for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                        else   
                            axs[1].plot([z.num1 for z in x[j[1]:j[2]]], [z.num1 for z in y[j[1]:j[2]]], c=:steelblue);
                            axs[2].plot([z.num1 for z in x[j[1]:j[2]]], [z.num2 for z in y[j[1]:j[2]]], c=:steelblue);
                            axs[3].plot([z.num1 for z in x[j[1]:j[2]]], [z.num3 for z in y[j[1]:j[2]]], c=:steelblue);
                        end
                #if first objective value is zero, plot zero(es) for the interval in all objectives prior the first degree of Ban's normal form
                else
                    for n=1:max_degree_y-degy[j[1]]
                        if j[1]==j[2]
                            axs[n].scatter([z.num1 for z in x[j[1]:j[2]]], 0, s=3,c = :steelblue);
                        else   
                            axs[n].plot([z.num1 for z in x[j[1]:j[2]]], zeros(j[2]-j[1]+1), c=:steelblue);
                        end
                    end
                    n=BAN.SIZE-(max_degree_y-degy[j[1]])
                    if n==1
                        if j[1]==j[2]
                            axs[3].scatter([z.num1 for z in x[j[1]:j[2]]], [z.num1 for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                        else   
                            axs[3].plot([z.num1 for z in x[j[1]:j[2]]], [z.num1 for z in y[j[1]:j[2]]], c=:steelblue);
                        end
                    elseif n==2 
                        if j[1]==j[2]
                            axs[2].scatter([z.num1 for z in x[j[1]:j[2]]], [z.num1 for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                            axs[3].scatter([z.num1 for z in x[j[1]:j[2]]], [z.num2 for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                        else   
                            axs[2].plot([z.num1 for z in x[j[1]:j[2]]], [z.num1 for z in y[j[1]:j[2]]], c=:steelblue);
                            axs[3].plot([z.num1 for z in x[j[1]:j[2]]], [z.num2 for z in y[j[1]:j[2]]], c=:steelblue);
                        end
                    end
                end
            end
            ...
            for j in intervals_y
                if j[1]==j[2]
                    axs[max_degree_y-degy[j[1]]+1].scatter([z.num1 for z in x[j[1]:j[2]]], [z.num1 for z in y[j[1]:j[2]]], s=3,c = :steelblue);
                else   
                    axs[max_degree_y-degy[j[1]]+1].plot([z.num1 for z in x[j[1]:j[2]]], [z.num1 for z in y[j[1]:j[2]]], c=:steelblue);
                end
            end
        end
