function GAA_CostFunction(x)
    ...
    NOISE2_CV < 0 && (NOISE2_CV = 0);
    WEMP2_CV < 0 && (WEMP2_CV = 0);
    DOC2_CV < 0 && (DOC2_CV = 0);
    ROUGH2_CV < 0 && (ROUGH2_CV = 0);
    WFUEL2_CV < 0 && (WFUEL2_CV = 0);
    RANGE2_CV < 0 && (RANGE2_CV = 0);
    NOISE4_CV < 0 && (NOISE4_CV = 0);
    WEMP4_CV < 0 && (WEMP4_CV = 0);
    DOC4_CV < 0 && (DOC4_CV = 0);
    ROUGH4_CV < 0 && (ROUGH4_CV = 0);
    WFUEL4_CV < 0 && (WFUEL4_CV = 0);
    RANGE4_CV < 0 && (RANGE4_CV = 0);
    NOISE6_CV < 0 && (NOISE6_CV = 0);
    WEMP6_CV < 0 && (WEMP6_CV = 0);
    DOC6_CV < 0 && (DOC6_CV = 0);
    ROUGH6_CV < 0 && (ROUGH6_CV = 0);
    WFUEL6_CV < 0 && (WFUEL6_CV = 0);
    RANGE6_CV < 0 && (RANGE6_CV = 0);
    ...
    WEMP2_DEV < 0 && (WEMP2_DEV = 0);
    DOC2_DEV < 0 && (DOC2_DEV = 0);
    WFUEL2_DEV < 0 && (WFUEL2_DEV = 0);
    PURCH2_DEV < 0 && (PURCH2_DEV = 0);
    RANGE2_DEV < 0 && (RANGE2_DEV = 0);
    LDMAX2_DEV < 0 && (LDMAX2_DEV = 0);
    VCMAX2_DEV < 0 && (VCMAX2_DEV = 0);
    WEMP4_DEV < 0 && (WEMP4_DEV = 0);
    DOC4_DEV < 0 && (DOC4_DEV = 0);
    WFUEL4_DEV < 0 && (WFUEL4_DEV = 0);
    PURCH4_DEV < 0 && (PURCH4_DEV = 0);
    RANGE4_DEV < 0 && (RANGE4_DEV = 0);
    LDMAX4_DEV < 0 && (LDMAX4_DEV = 0);
    VCMAX4_DEV < 0 && (VCMAX4_DEV = 0);
    WEMP6_DEV < 0 && (WEMP6_DEV = 0);
    DOC6_DEV < 0 && (DOC6_DEV = 0);
    WFUEL6_DEV < 0 && (WFUEL6_DEV = 0);
    PURCH6_DEV < 0 && (PURCH6_DEV = 0);
    RANGE6_DEV < 0 && (RANGE6_DEV = 0);
    LDMAX6_DEV < 0 && (LDMAX6_DEV = 0);
    VCMAX6_DEV < 0 && (VCMAX6_DEV = 0);
    
    max_NOISE = max(NOISE2, NOISE4, NOISE6);
    max_WEMP = max(WEMP2, WEMP4, WEMP6);
    max_DOC = max(DOC2, DOC4, DOC6);
    max_ROUGH = max(ROUGH2, ROUGH4, ROUGH6);
    max_WFUEL = max(WFUEL2, WFUEL4, WFUEL6);
    max_PURCH = max(PURCH2, PURCH4, PURCH6);
    min_RANGE = min(RANGE2, RANGE4, RANGE6);
    min_LDMAX = min(LDMAX2, LDMAX4, LDMAX6);
    min_VCMAX = min(VCMAX2, VCMAX4, VCMAX6);

    ##Objectives for the non-scaled formulation
    Cost = [max_NOISE, max_WEMP, max_DOC, max_ROUGH, max_WFUEL, max_PURCH, -min_RANGE, -min_LDMAX, -min_VCMAX, PFPF]
    
    return Cost, CV_TOTAL
end

function GAA_CostFunction_Primaries(x)
    ...
    CV_TOTAL = DOC2_CV + RANGE2_CV + DOC4_CV + RANGE4_CV + DOC6_CV + RANGE6_CV;
    
    if !(CV_TOTAL ≈ 0)
        return zeros(n1), CV_TOTAL
    end
    ...
    ##Objectives for the non-scaled formulation
    Cost = [max_DOC, -min_RANGE, -min_LDMAX, PFPF]
