...
function plot_level(P, gen::Int=0)
    ...
    scatter3D(map(x -> x.y[2][0], P), map(x -> x.y[3][0], P), map(x -> x.y[4][0], P), marker=:x)
end
...
function MPL_GAA(filename)
    ...
    open(filename, "w") do  file
        ...
        println(file, n)
        for x in EP
            println(file, string(x.y[1][0], ';', x.y[2][0], ';', x.y[3][0], ';', x.y[4][0]))
            println(file, string(x.y[2][-1], ';', x.y[3][-1], ';', x.y[4][-1]))
            println(file, string(x.y[2][-2], ';', x.y[3][-2], ';', x.y[4][-2]))
        end
    end
end